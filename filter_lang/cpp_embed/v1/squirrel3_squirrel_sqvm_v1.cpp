...
/**
 * @brief スタックのトップにある値を取得します。
 */
#define TOP() (_stack._vals[_top-1])
/**
 * @brief 命令のターゲットレジスタ（スタック上の特定の場所）を取得します。
 */
#define TARGET _stack._vals[_stackbase+arg0]
/**
 * @brief 現在のスタックベースからの相対位置でスタック上の値を取得します。
 * @param a スタックベースからのオフセット。
 */
#define STK(a) _stack._vals[_stackbase+(a)]

/**
 * @brief 2つの整数オブジェクトに対してビット単位の演算を実行します。
 * @details この関数は、AND, OR, XOR, 左シフト, 右シフト, 符号なし右シフトのビット演算をサポートします。
 * 両方のオペランドが整数でない場合はエラーを発生させます。
 * @param op 実行するビット演算の種類を示す演算子コード。
 * @param[out] trg 演算結果を格納するSQObjectPtr。
 * @param[in] o1 最初のオペランド。
 * @param[in] o2 2番目のオペランド。
 * @return 演算が成功した場合はtrue、そうでなければfalse。
 */
...
/**
 * @brief 算術演算を実行するためのマクロ。
 * @details オブジェクトの型に応じて整数または浮動小数点数の演算を高速に実行します。
 * それ以外の型の場合はARITH_OP関数にフォールバックします。
 * @param op 算術演算子。
 * @param trg 結果を格納するSQObjectPtr。
 * @param o1 最初のオペランド。
 * @param o2 2番目のオペランド。
 */
...
/**
 * @brief ゼロ除算チェック付きの算術演算を実行するためのマクロ。
 * @details _ARITH_マクロに似ていますが、整数除算の際にゼロ除算をチェックし、エラーを発生させます。
 * @param op 算術演算子。
 * @param trg 結果を格納するSQObjectPtr。
 * @param o1 最初のオペランド。
 * @param o2 2番目のオペランド。
 * @param err ゼロ除算の場合のエラーメッセージ。
 */
...
/**
 * @brief 2つのオブジェクトに対して算術演算を実行します。
 * @details この関数は、整数、浮動小数点数、文字列の加算（連結）を処理します。
 * サポートされていない型の場合は、メタメソッドの呼び出しを試みます。
 * @param op 実行する算術演算の種類を示す文字（'+', '-', '/', '*', '%'）。
 * @param[out] trg 演算結果を格納するSQObjectPtr。
 * @param[in] o1 最初のオペランド。
 * @param[in] o2 2番目のオペランド。
 * @return 演算が成功した場合はtrue、そうでなければfalse。
...
/**
 * @brief SQVMのコンストラクタ。
 * @details 仮想マシンのメンバ変数を初期状態に設定します。
 * @param ss 共有状態(SharedState)へのポインタ。
 */
...
/**
 * @brief 仮想マシンをファイナライズし、リソースを解放します。
 * @details 開いているアウター、ルートテーブル、エラーハンドラなどをクリアし、スタックを空にします。
 */
void SQVM::Finalize()
{
...
/**
 * @brief SQVMのデストラクタ。
 * @details Finalizeを呼び出してリソースを解放し、GCチェーンから自身を削除します。
 */
SQVM::~SQVM()
{
    Finalize();
...
/**
 * @brief 算術演算のメタメソッドを呼び出します。
 * @details オブジェクトにデリゲートが設定されており、対応する算術メタメソッド（_add, _subなど）が存在する場合、それを呼び出します。
 * @param op 実行する算術演算の種類を示す文字。
 * @param[in] o1 最初のオペランド。
 * @param[in] o2 2番目のオペランド。
 * @param[out] dest 演算結果を格納するSQObjectPtr。
 * @return メタメソッドの呼び出しが成功した場合はtrue、そうでなければfalse。
 */
...
/**
 * @brief オブジェクトの符号を反転させます（単項マイナス演算）。
 * @details オペランドが整数または浮動小数点数の場合は、その値を反転させます。
 * テーブル、ユーザーデータ、インスタンスの場合は、_unmメタメソッドの呼び出しを試みます。
 * @param[out] trg 演算結果を格納するSQObjectPtr。
 * @param[in] o 符号を反転させるオペランド。
 * @return 演算が成功した場合はtrue、そうでなければfalse。
 */
...
/**
 * @brief 比較結果を設定してtrueを返すマクロ。
 * @details ObjCmp関数内で使用され、コードを簡潔にします。
 * @param exp 比較結果の式。
 */
#define _RET_SUCCEED(exp) { result = (exp); return true; }
/**
 * @brief 2つのSquirrelオブジェクトを比較します。
 * @details オブジェクトの型と値に基づいて比較を行います。
 * 同じ型の場合は値を直接比較し、数値型同士の場合は型変換後に比較します。
 * テーブル、ユーザーデータ、インスタンスの場合は、_cmpメタメソッドの呼び出しを試みます。
 * @param[in] o1 最初の比較オブジェクト。
 * @param[in] o2 2番目の比較オブジェクト。
 * @param[out] result 比較結果を格納する整数への参照。o1 < o2なら-1、o1 == o2なら0、o1 > o2なら1。
 * @return 比較が成功した場合はtrue、そうでなければfalse。
 */
...
/**
 * @brief 比較演算（<, <=, >, >=, <=>)を実行します。
 * @details ObjCmpを呼び出して2つのオブジェクトを比較し、その結果に基づいて指定された比較演算の真偽値を返します。
 * @param op 実行する比較演算の種類。
 * @param[in] o1 最初のオペランド。
 * @param[in] o2 2番目のオペランド。
 * @param[out] res 演算結果（真偽値または整数）を格納するSQObjectPtr。
 * @return 演算が成功した場合はtrue、そうでなければfalse。
 */
...
/**
 * @brief オブジェクトを文字列に変換します。
 * @details 組み込み型（数値、ブール値、null）を対応する文字列に変換します。
 * テーブル、ユーザーデータ、インスタンスの場合は、_tostringメタメソッドの呼び出しを試みます。
 * メタメソッドがない場合は、型の名前とポインタアドレスからなるデフォルトの文字列を生成します。
 * @param[in] o 文字列に変換するオブジェクト。
 * @param[out] res 変換後の文字列を格納するSQObjectPtr。
 * @return 変換が成功した場合はtrue、そうでなければfalse。
 */
...
/**
 * @brief 2つのオブジェクトを文字列として連結します。
 * @details 両方のオブジェクトをToStringで文字列に変換し、それらを連結して新しい文字列オブジェクトを作成します。
 * @param[in] str 最初のオブジェクト（文字列に変換される）。
 * @param[in] obj 2番目のオブジェクト（文字列に変換される）。
 * @param[out] dest 連結後の文字列を格納するSQObjectPtr。
 * @return 連結が成功した場合はtrue、そうでなければfalse。
 */
 ...
 * @details オブジェクトに_typeofメタメソッドが定義されている場合はそれを呼び出します。
 * そうでない場合は、組み込みの型名を返します。
 * @param[in] obj1 型名を取得するオブジェクト。
 * @param[out] dest 型名文字列を格納するSQObjectPtr。
 * @return 処理が成功した場合はtrue、そうでなければfalse。
 */
...
/**
 * @brief 仮想マシンを初期化します。
 * @details スタック、コールスタック、ルートテーブルなどをセットアップします。
 * friendvmが指定された場合、そのVMと一部の状態（ルートテーブル、エラーハンドラなど）を共有します。
 * @param friendvm 状態を共有するオプションの既存VM。
 * @param stacksize 初期スタックサイズ。
...
/**
 * @brief Squirrel関数呼び出しを開始します。
 * @details 新しいコールフレームを設定し、引数を処理し（可変長引数とデフォルト引数を含む）、
 * 呼び出される関数の実行コンテキストを初期化します。
 * ジェネレータ関数の場合は、ジェネレータオブジェクトを作成して中断（yield）します。
 * @param closure 呼び出すクロージャ。
 * @param target 戻り値を格納するスタック上のターゲットインデックス。
 * @param args 渡された引数の数。
 * @param stackbase 新しいコールフレームのスタックベースインデックス。
 * @param tailcall この呼び出しが末尾最適化の対象であるかどうか。
 * @return 呼び出しのセットアップが成功した場合はtrue、そうでなければfalse。
 */
...
/**
 * @brief 関数からの復帰を処理します。
 * @details 呼び出し元に復帰値を設定し、現在のコールフレームを破棄します。
 * デバッグフックが有効な場合は、復帰イベントを通知します。
 * @param _arg0 戻り値があるかどうかを示すフラグ (0xFFの場合は戻り値なし)。
 * @param _arg1 スタック上の戻り値のインデックス。
 * @param[out] retval ルートコールからの復帰である場合に、最終的な戻り値を格納します。
 * @return ルートコールからの復帰の場合はtrue、そうでなければfalse。
 */
...
/**
 * @brief 式の評価が失敗した場合にfalseを返すマクロ。
 * @param exp 評価する式。
 */
#define _RET_ON_FAIL(exp) { if(!exp) return false; }

/**
 * @brief ローカル変数の後置インクリメント/デクリメントを処理します。
 * @details 変数の値を変更し、変更前の値をターゲットに設定します。
 * @param op 演算子（'+'または'-'）。
 * @param[out] target 演算前の値を格納するオブジェクト。
 * @param[in,out] a 変更対象のローカル変数。
 * @param[in] incr インクリメント/デクリメントする値。
 * @return 演算が成功した場合はtrue、そうでなければfalse。
 */
...
/**
 * @brief 間接参照された値（テーブルの要素やインスタンスのメンバーなど）のインクリメント/デクリメントを処理します。
 * @details オブジェクトからキーを使って値を取得し、演算を行い、結果を書き戻します。
 * 前置演算と後置演算の両方をサポートします。
 * @param op 演算子（'+'または'-'）。
 * @param[out] target 演算結果を格納するオブジェクト。後置演算の場合は演算前の値が格納される。
 * @param[in] self 対象となるオブジェクト（テーブルなど）。
 * @param[in] key 値を取得するためのキー。
 * @param[in] incr インクリメント/デクリメントする値。
 * @param postfix 後置演算の場合はtrue、前置演算の場合はfalse。
 * @param selfidx selfオブジェクトのスタックインデックス。
 * @return 演算が成功した場合はtrue、そうでなければfalse。
 */
...
/** @brief 現在の命令の0番目の引数（符号付き）。 */
#define sarg0 ((SQInteger)*((const signed char *)&_i_._arg0))
/** @brief 現在の命令の1番目の引数。 */
#define arg1 (_i_._arg1)
/** @brief 現在の命令の1番目の引数（符号付き）。 */
#define sarg1 (*((const SQInt32 *)&_i_._arg1))
/** @brief 現在の命令の2番目の引数。 */
#define arg2 (_i_._arg2)
/** @brief 現在の命令の3番目の引数。 */
#define arg3 (_i_._arg3)
/** @brief 現在の命令の3番目の引数（符号付き）。 */
#define sarg3 ((SQInteger)*((const signed char *)&_i_._arg3))

/**
 * @brief 仮想マシンを中断します。
 * @details VMがすでに中断されている場合や、ネイティブコール/メタメソッド呼び出しのネストが深すぎる場合はエラーを返します。
 * @return 成功した場合はSQ_SUSPEND_FLAG、失敗した場合はエラーをスローします。
 */
...
/**
 * @brief foreachループの反復処理を終了し、ジャンプオフセットを設定するマクロ。
 * @param howmuchtojump IPを進めるオフセット値。
 */
#define _FINISH(howmuchtojump) {jump = howmuchtojump; return true; }
/**
 * @brief foreachループの1ステップを実行します。
 * @details イテレート対象のオブジェクトの種類に応じて、次のインデックスと値を取得します。
 * テーブル、配列、文字列、クラス、ジェネレータ、および_nextiメタメソッドを持つオブジェクトのイテレーションをサポートします。
 * @param o1 イテレート対象のオブジェクト。
 * @param[out] o2 イテレータのインデックスを格納するオブジェクト。
 * @param[out] o3 イテレータの値を格納するオブジェクト。
 * @param[in,out] o4 イテレーションの状態を保持するオブジェクト。
 * @param arg_2 未使用の引数。
 * @param exitpos ループの終端に到達した場合のジャンプオフセット。
 * @param[out] jump 次の命令へのジャンプオフセット(1:継続, exitpos:終了)。
...
/** @brief 条件付きでリテラルまたはスタック上の値を取得するマクロ。 */
#define COND_LITERAL (arg3!=0?ci->_literals[arg1]:STK(arg1))

/** @brief 例外トラップへジャンプするマクロ。 */
#define SQ_THROW() { goto exception_trap; }

/** @brief 式を評価し、失敗した場合に例外をスローするマクロ。 */
#define _GUARD(exp) { if(!exp) { SQ_THROW();} }

/**
 * @brief 新しいクロージャオブジェクトを作成します。
 * @details 関数プロトタイプからクロージャを生成し、アウターバリュー（アップバリュー）をキャプチャし、
 * デフォルト引数を設定します。オプションで環境オブジェクト（this）を束縛することもできます。
 * @param[out] target 作成されたクロージャを格納するオブジェクト。
 * @param func クロージャの基となる関数プロトタイプ。
 * @param boundtarget 環境として束縛するオブジェクトのスタックインデックス (0xFFの場合はなし)。
 * @return クロージャの作成が成功した場合はtrue、そうでなければfalse。
 */
...
/**
 * @brief 新しいクラスオブジェクトを作成します。
 * @details オプションで基底クラスを継承し、属性テーブルを設定します。
 * 基底クラスに_inheritedメタメソッドが定義されている場合は、それを呼び出します。
 * @param[out] target 作成されたクラスオブジェクトを格納するオブジェクト。
 * @param baseclass 基底クラスのスタックインデックス (-1の場合はなし)。
 * @param attributes 属性テーブルのスタックインデックス (MAX_FUNC_STACKSIZEの場合はなし)。
 * @return クラスの作成が成功した場合はtrue、そうでなければfalse。
 */
...
/**
 * @brief 2つのオブジェクトが等しいかどうかを判断します（==演算子）。
 * @details 型が同じ場合は生の値を比較します。数値型同士の場合は浮動小数点数に変換して比較します。
 * @param[in] o1 最初のオブジェクト。
 * @param[in] o2 2番目のオブジェクト。
 * @param[out] res 比較結果（trueまたはfalse）を格納するbool参照。
 * @return 常にtrueを返します（この操作自体は失敗しません）。
 */
...
/**
 * @brief オブジェクトがSquirrelの論理評価でfalseと見なされるかどうかを判断します。
 * @details null、整数0、浮動小数点数0.0、ブール値falseがfalseと見なされます。
 * @param o 評価するオブジェクト。
 * @return オブジェクトがfalseと見なされる場合はtrue、そうでなければfalse。
 */
...
/**
 * @brief 命令セットのディスクリプション配列（外部リンケージ）。デバッグ用。
 */
extern SQInstructionDesc g_InstrDesc[];

/**
 * @brief Squirrelクロージャの実行を開始し、VMのメインループを駆動します。
 * @details この関数はSquirrel VMの心臓部です。指定されたクロージャを実行するためにVMの状態をセットアップし、
 * バイトコード命令を一つずつ解釈・実行するループを開始します。
 * 実行の種類（新規呼び出し、ジェネレータの再開など）に応じて初期化処理を行った後、
 * 命令ディスパッチループに入ります。ループ内では、各オペコードに対応する処理を実行します。
 * エラーが発生した場合は、exception_trapブロックで例外を処理し、スタックを巻き戻します。
 * @param closure 実行するクロージャオブジェクト。
 * @param nargs 関数に渡される引数の数。
 * @param stackbase この呼び出しのスタックベースインデックス。
 * @param[out] outres 実行が正常に完了した場合の戻り値を格納するオブジェクト。
 * @param raiseerror エラー発生時にエラーハンドラを呼び出すかどうか。
 * @param et 実行の種類（通常呼び出し、ジェネレータ再開など）。
 * @return 実行が正常に完了した場合はtrue、エラーが発生した場合はfalse。
 */
...
exception_restore:
    //
    {
        for(;;)
        {
            ...
            // 命令のオペコードに基づき、対応する処理を実行するディスパッチループ
            switch(_i_.op)
            {
...
//      SQInteger n = 0;
        ...
        while( ci ) {
            if(ci->_etraps > 0) {
                // 例外トラップ（try-catch）が見つかった場合
                SQExceptionTrap &et = _etraps.top();
                ci->_ip = et._ip; // IPをハンドラのアドレスに設定
                _top = et._stacksize;
                _stackbase = et._stackbase;
                _stack._vals[_stackbase + et._extarget] = currerror; // エラーオブジェクトをスタックに積む
                _etraps.pop_back(); traps--; ci->_etraps--;
                while(last_top >= _top) _stack._vals[last_top--].Null();
                goto exception_restore; // 実行ループに復帰
            }
            else if (_debughook) {
            ...
            LeaveFrame(); // 現在のフレームを破棄してスタックを巻き戻す
            if(mustbreak) break;
        }
...
/**
 * @brief 指定されたクラスのインスタンスを作成します。
 * @details クラスから新しいインスタンスを生成し、クラスにコンストラクタが定義されていればそれを取得します。
 * @param theclass インスタンスを作成するクラス。
 * @param[out] inst 作成された新しいインスタンスを格納するオブジェクト。
 * @param[out] constructor クラスのコンストラクタが存在する場合、それを格納するオブジェクト。
 * @return 常にtrueを返します。
 */
...
/**
 * @brief VMに登録されたエラーハンドラを呼び出します。
 * @details エラーハンドラが設定されている場合、ルートテーブルとエラーオブジェクトをスタックにプッシュし、
 * エラーハンドラ関数を呼び出します。
 * @param error エラーハンドラに渡すエラーオブジェクト。
 */
...
/**
 * @brief VMに登録されたデバッグフックを呼び出します。
 * @details デバッグフックは、ネイティブ関数またはSquirrelクロージャとして登録できます。
 * この関数は、再帰呼び出しを防ぐために一時的にフックを無効にしてから、登録されたフックを呼び出します。
 * @param type デバッグイベントの種類（'c': call, 'l': line, 'r': returnなど）。
 * @param forcedline 強制的に使用する行番号。0の場合は現在の命令ポインタから行番号を取得します。
 */
...
/**
 * @brief ネイティブC/C++関数を呼び出します。
 * @details ネイティブクロージャを実行するための準備と後処理を行います。
 * 引数の数や型のチェック、コールフレームの設定、C関数の呼び出し、戻り値の処理、
 * そしてVMの中断や末尾呼び出し要求のハンドリングが含まれます。
 * @param nclosure 呼び出すネイティブクロージャ。
 * @param nargs 渡された引数の数。
 * @param newbase 新しいコールフレームのスタックベース。
 * @param[out] retval 戻り値を格納するオブジェクト。
 * @param target 戻り値を格納するスタックのターゲットインデックス。
 * @param[out] suspend ネイティブ関数がVMの中断を要求した場合にtrueが設定されます。
 * @param[out] tailcall ネイティブ関数が末尾呼び出しを要求した場合にtrueが設定されます。
 * @return 呼び出しが成功した場合はtrue、そうでなければfalse。
 */
...
/**
 * @brief 末尾呼び出しを実行します。
 * @details 現在のスタックフレームを再利用して新しい関数を呼び出すことで、スタックの消費を防ぎます。
 * ルートコールからの末尾呼び出しは許可されていません。
 * @param closure 末尾呼び出しするクロージャ。
 * @param parambase 新しい呼び出しの引数が配置されているスタックベース。
 * @param nparams 新しい呼び出しの引数の数。
 * @return 呼び出しが成功した場合はtrue、そうでなければfalse。
 */
...
/** @brief フォールバックGet/Set操作が成功したことを示す定数。 */
#define FALLBACK_OK         0
/** @brief フォールバックGet/Set操作で一致するものがなかったことを示す定数。 */
#define FALLBACK_NO_MATCH   1
/** @brief フォールバックGet/Set操作中にエラーが発生したことを示す定数。 */
#define FALLBACK_ERROR      2

/**
 * @brief オブジェクトからキーを使って値を取得します。
 * @details テーブル、配列、インスタンス、クラス、文字列など、様々なオブジェクト型からの値の取得を処理します。
 * 直接の値取得に失敗した場合、デリゲート、メタメソッド(_get)、デフォルトデリゲートなどのフォールバック機構を試みます。
 * @param self 値を取得する対象のオブジェクト。
 * @param key 取得する値のキー。
 * @param[out] dest 取得した値を格納するオブジェクト。
 * @param getflags 取得処理の挙動を制御するフラグ。
 * @param selfidx selfオブジェクトのスタックインデックス。
 * @return 値の取得に成功した場合はtrue、そうでなければfalse。
 */
...
/**
 * @brief オブジェクトの型に対応するデフォルトデリゲートから値を取得します。
 * @details 各組み込み型（数値、文字列など）には、共通のメソッドを持つデフォルトデリゲートテーブルが関連付けられています。
 * この関数は、そのテーブルからキーに対応する値（通常はメソッド）を取得します。
 * @param self デフォルトデリゲートを決定するためのオブジェクト。
 * @param key 取得する値のキー。
 * @param[out] dest 取得した値を格納するオブジェクト。
 * @return 値の取得に成功した場合はtrue、そうでなければfalse。
 */
...
/**
 * @brief Get操作のフォールバック処理を実行します。
 * @details オブジェクトから直接キーが見つからなかった場合に呼び出されます。
 * オブジェクトのデリゲートを再帰的に検索するか、_getメタメソッドの呼び出しを試みます。
 * @param self 値を取得する対象のオブジェクト。
 * @param key 取得する値のキー。
 * @param[out] dest 取得した値を格納するオブジェクト。
 * @return 処理の成功、失敗、または該当なしを示すフォールバックステータスコード。
 */
...
/**
 * @brief オブジェクトのキーに値を設定します。
 * @details テーブル、インスタンス、配列などのオブジェクトへの値の設定を処理します。
 * 直接の設定ができない、または失敗した場合、デリゲート、メタメソッド(_set)などのフォールバック機構を試みます。
 * @param self 値を設定する対象のオブジェクト。
 * @param key 設定する値のキー。
 * @param val 設定する値。
 * @param selfidx selfオブジェクトのスタックインデックス。
 * @return 値の設定に成功した場合はtrue、そうでなければfalse。
 */
...
/**
 * @brief Set操作のフォールバック処理を実行します。
 * @details オブジェクトに直接値を設定できなかった場合に呼び出されます。
 * オブジェクトのデリゲートに再帰的に設定を試みるか、_setメタメソッドの呼び出しを試みます。
 * @param self 値を設定する対象のオブジェクト。
 * @param key 設定する値のキー。
 * @param val 設定する値。
 * @return 処理の成功、失敗、または該当なしを示すフォールバックステータスコード。
...
/**
 * @brief オブジェクトのクローン（シャローコピー）を作成します。
 * @details テーブル、インスタンス、配列のクローンを作成できます。
 * クローン作成後、新しいオブジェクトに_clonedメタメソッドが定義されていれば、それを呼び出します。
 * @param self クローンする元のオブジェクト。
 * @param[out] target 作成された新しいオブジェクトを格納します。
 * @return クローン作成が成功した場合はtrue、サポートされていない型の場合はfalse。
 */
...
/**
 * @brief クラスに新しいスロット（メンバー）を属性付きで追加します。
 * @details `class.newmember()`構文の実装です。クラスに_newmemberメタメソッドが定義されている場合、
 * 実際の追加処理の代わりにメタメソッドを呼び出します。
 * @param self スロットを追加するクラスオブジェクト。
 * @param key 新しいスロットのキー。
 * @param val 新しいスロットの値。
 * @param attrs スロットに関連付ける属性テーブル（オプショナル）。
 * @param bstatic スロットを静的メンバーとして追加する場合はtrue。
 * @param raw rawモード。trueの場合、_newmemberメタメソッドをバイパスします。
 * @return スロットの追加が成功した場合はtrue、そうでなければfalse。
 */
...
/**
 * @brief オブジェクトに新しいスロット（キーと値のペア）を追加します。
 * @details `<-` 演算子の実装です。テーブルやクラスに新しいメンバーを追加するために使用されます。
 * テーブルの場合、キーがまだ存在しなければ_newslotメタメソッドの呼び出しを試みます。
 * クラスの場合、静的またはインスタンスメンバーとして新しいスロットを作成します。
 * インスタンスに直接新しいスロットを追加しようとするとエラーになります。
 * @param self スロットを追加する対象のオブジェクト（テーブルまたはクラス）。
 * @param key 新しいスロットのキー。
 * @param val 新しいスロットの値。
 * @param bstatic クラスの場合、スロットが静的メンバーかどうかを示します。
 * @return スロットの追加に成功した場合はtrue、そうでなければfalse。
 */
...
/**
 * @brief オブジェクトから指定されたキーを持つスロットを削除します。
 * @details `delete` 演算子の実装です。テーブル、インスタンス、ユーザーデータからスロットを削除します。
 * 最初に_delslotメタメソッドの呼び出しを試み、失敗した場合はテーブルから直接要素を削除します。
 * @param self スロットを削除する対象のオブジェクト。
 * @param key 削除するスロットのキー。
 * @param[out] res 削除されたスロットの値を格納するオブジェクト。
 * @return 削除に成功した場合はtrue、そうでなければfalse。
 */
...
/**
 * @brief 任意の呼び出し可能オブジェクト（クロージャ、ネイティブクロージャ、クラス）を呼び出します。
 * @details この関数は、呼び出すオブジェクトの型を判別し、適切な実行関数（ExecuteやCallNativeなど）に処理をディスパッチします。
 * クラスが呼び出された場合は、インスタンスを生成しコンストラクタを呼び出します。
 * @param closure 呼び出すクロージャ、ネイティブクロージャ、またはクラス。
 * @param nparams 渡された引数の数。
 * @param stackbase 呼び出しのスタックベースインデックス。
 * @param[out] outres 呼び出しの戻り値を格納するオブジェクト。
 * @param raiseerror エラー発生時にエラーハンドラを呼び出すかどうか。
 * @return 呼び出しが成功した場合はtrue、そうでなければfalse。
 */
...
/**
 * @brief メタメソッドを安全に呼び出すためのラッパー関数。
 * @details メタメソッド呼び出しのネストレベルを管理し、Call関数を呼び出します。
 * 呼び出し後、スタックから引数をポップします。
 * @param closure 呼び出すメタメソッドのクロージャ。
 * @param mm 呼び出すメタメソッドのID（現在は未使用）。
 * @param nparams メタメソッドに渡す引数の数。
 * @param[out] outres メタメソッドの戻り値を格納するオブジェクト。
 * @return メタメソッドの呼び出しが成功した場合はtrue、そうでなければfalse。
 */
...
/**
 * @brief アウターバリュー（アップバリュー）を検索または作成します。
 * @details クロージャが自身を囲む関数のローカル変数を参照する場合、その変数は「アウターバリュー」としてキャプチャされます。
 * この関数は、指定されたスタック位置に対応するアウターバリューが既に存在するかをチェックし、
 * 存在すればそれを返し、存在しなければ新しく作成してVMのオープンアウターリストに追加します。
 * @param[out] target 発見または作成されたアウターオブジェクトを格納します。
 * @param stackindex キャプチャするスタック上のローカル変数へのポインタ。
 */
...
/**
 * @brief 新しいコールフレームに入ります。
 * @details 関数呼び出し時に、新しい実行コンテキスト（コールフレーム）をコールスタックにプッシュします。
 * 呼び出し元のスタックベースやトップ位置を保存し、VMのスタックポインタを更新します。
 * 必要に応じてオペランドスタックのサイズを拡張します。末尾呼び出しの場合は既存のフレームを再利用します。
 * @param newbase 新しいフレームのスタックベース。
 * @param newtop 新しいフレームのスタックトップ。
 * @param tailcall 末尾呼び出しの場合はtrue。
 * @return 成功した場合はtrue、スタックオーバーフローの場合はfalse。
 */
...
/**
 * @brief 現在のコールフレームから抜けます。
 * @details 関数からのリターン時に呼び出され、コールスタックから現在のフレームをポップし、
 * 呼び出し元のフレームの状態を復元します。また、スコープ外に出るアウターバリューをクローズします。
 */
...
/**
 * @brief スタックがリサイズされた後にアウターバリューのポインタを再配置します。
 * @details オペランドスタックがメモリ上で移動した場合、開いているすべてのアウターバリューが持つ
 * スタック上の値へのポインタを、新しいメモリアドレスを指すように更新します。
 */
...
/**
 * @brief 指定されたスタック位置以上にある開いているアウターバリューをクローズします。
 * @details 変数がスコープ外に出る際に、その変数をキャプチャしているアウターバリューを「クローズ」します。
 * これにより、スタック上の値がアウターオブジェクト自体にコピーされ、
 * スタックフレームが破棄された後も値が存続するようになります。
 * @param stackindex このスタックアドレス以上の範囲にあるアウターをクローズします。
 */
...
/**
 * @brief スタック上の指定されたインデックスの要素を削除します。
 * @param n 削除する要素のインデックス。
 */
...
/**
 * @brief スタックのトップから指定された数の要素をポップします。
 * @param n ポップする要素の数。
 */
...
/** @brief スタックにnullをプッシュします。 */
void SQVM::PushNull() { _stack[_top++].Null(); }
/** @brief スタックにオブジェクトをプッシュします。 */
void SQVM::Push(const SQObjectPtr &o) { _stack[_top++] = o; }
/** @brief スタックのトップにあるオブジェクトへの参照を取得します。 */
SQObjectPtr &SQVM::Top() { return _stack[_top-1]; }
/** @brief スタックからオブジェクトをポップし、その参照を取得します。 */
SQObjectPtr &SQVM::PopGet() { return _stack[--_top]; }
/** @brief スタックトップからの相対位置(上方)にあるオブジェクトへの参照を取得します。 */
SQObjectPtr &SQVM::GetUp(SQInteger n) { return _stack[_top+n]; }
/** @brief スタックの絶対位置にあるオブジェクトへの参照を取得します。 */
SQObjectPtr &SQVM::GetAt(SQInteger n) { return _stack[n]; }

#ifdef _DEBUG_DUMP
/**
 * @brief スタックの内容をコンソールにダンプします（デバッグ用）。
 * @param stackbase 現在のスタックベース。
 * @param dumpall trueの場合、確保されている全スタックを、falseの場合はトップまでのスタックをダンプします。
 */
